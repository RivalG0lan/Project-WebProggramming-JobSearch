...
$query_lowongan = mysqli_query($conn,
...
foreach(['dikirim','review','interview','accepted','rejected'] as $s){
    ...
    if($filter_lowongan > 0) $w2 .= " AND lmr.id_lowongan='$filter_lowongan'";
    $r = mysqli_query($conn,
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .employer-score-fill { height:100%; background:#f59e0b; width:50%; border-radius:2px; }
        nav { flex:1; }
        .nav-item {
        ...
        .greeting { font-size:16px; color:#6b7280; }
        .btn-post {
        ...
        .btn-post:hover { background:#0f766e; }
        ...
        .stat-pill:hover { border-color:#0d9488; color:#0d9488; }
        .stat-pill.active { background:#e0f2f1; border-color:#0d9488; color:#0d9488; }
        .stat-pill .pill-num {
            background:#f3f4f6; color:#111827; padding:2px 8px;
            border-radius:12px; font-size:12px;
        }
        .stat-pill.active .pill-num { background:#0d9488; color:white; }
        ...
        .btn-filter-submit:hover { background:#0f766e; }
        ...
        .ai-banner svg { color:#0d9488; flex-shrink:0; }
        .ai-banner-title { font-size:15px; font-weight:700; color:#111827; }
        .ai-banner-sub { font-size:13px; color:#6b7280; }
        ...
        .cand-avatar {
            width:52px; height:52px; border-radius:50%; background:#0d9488;
            display:flex; align-items:center; justify-content:center;
            color:white; font-weight:700; font-size:18px; flex-shrink:0;
        }
        .cand-info { flex:1; }
        .cand-name-row { display:flex; align-items:center; gap:10px; margin-bottom:4px; flex-wrap:wrap; }
        .cand-name { font-size:18px; font-weight:600; color:#111827; }
        .cand-pos { font-size:14px; color:#6b7280; margin-bottom:8px; }
        .cand-meta {
            display:flex; align-items:center; gap:16px; flex-wrap:wrap;
            font-size:13px; color:#6b7280;
        }
        .cand-meta-item { display:flex; align-items:center; gap:5px; }
        .cand-applied { margin-left:auto; font-size:12px; color:#9ca3af; white-space:nowrap; }
        ...
        .update-section label { font-size:13px; font-weight:600; color:#374151; white-space:nowrap; }
        .status-select {
            padding:8px 12px; border:1px solid #e5e7eb; border-radius:8px;
            font-size:14px; background:#f9fafb; cursor:pointer; outline:none;
            min-width:160px;
        }
        .status-select:focus { border-color:#0d9488; }
        .btn-update {
        ...
        .card-actions {
            ...
            padding-top:16px; border-top:1px solid #f3f4f6; margin-top:16px;
        }
        .btn-action {
            padding:8px 16px; border-radius:8px; font-size:13px; font-weight:600;
            cursor:pointer; border:1px solid; display:flex; align-items:center; gap:6px;
            transition:all 0.2s; text-decoration:none;
        }
        .btn-outline-teal { background:white; color:#0d9488; border-color:#0d9488; }
        .btn-outline-teal:hover { background:#f0fdfa; }
        .btn-outline-red  { background:white; color:#ef4444; border-color:#ef4444; }
...
<aside class="sidebar">
    ...
    <div class="employer-score">
        ...
        <div class="employer-score-bar"><div class="employer-score-fill"></div></div>
    </div>
    <nav>
        ...
        <a href="posting_lowongan.php" class="nav-item">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <circle cx="12" cy="12" r="1"/><circle cx="12" cy="12" r="8"/><line x1="12" y1="1" x2="12" y2="3"/>
...
<main class="main-content">
    ...
    <div class="stats-row">
        ...
        ?>
        <a href="kandidat.php<?= $q ?>"
           class="stat-pill <?= $filter_status === $sk ? 'active' : '' ?>">
    ...
    <form method="GET" action="kandidat.php">
        <div class="filter-bar">
            ...
            <select name="id_lowongan" class="filter-select">
                ...
                ?>
                <option value="<?= $lw['id_lowongan'] ?>"
                    <?= $filter_lowongan == $lw['id_lowongan'] ? 'selected' : '' ?>>
    ...
    <div class="ai-banner">
        ...
        <div>
            ...
            <div class="ai-banner-sub">Kandidat diurutkan berdasarkan waktu lamaran terbaru. Update status langsung dari kartu di bawah.</div>
        </div>
    </div>
